...
func (mem *Memory) AddMessage(msg Message) {
	mem.Messages = append(mem.Messages, msg)
}

func (mem *Memory) AddMessages(msgs []Message) {
	for _, m := range msgs {
		mem.AddMessage(m)
	}
}

func (mem *Memory) Save(fileName string) {
	memFile, err := os.Create(fileName)
	if err != nil {
		log.Printf("Error opening memory file: %v", err)
		return
	}
	defer memFile.Close()
	encoder := json.NewEncoder(memFile)
	encoder.Encode(mem.Messages)
}

func (mem *Memory) Load(fileName string) {
	memFile, err := os.Open(fileName)
	if err != nil {
		log.Printf("Error opening memory file: %v", err)
		return
	}
	defer memFile.Close()
	decoder := json.NewDecoder(memFile)
	err = decoder.Decode(&mem.Messages)
	if err != nil {
		log.Printf("Error decoding memory file: %v", err)
		return
	}
}
...
func main() {
	...
	for {
		...
		if strings.ToLower(userInput) == "exit" || strings.ToLower(userInput) == "quit" || strings.ToLower(userInput) == "/exit" || strings.ToLower(userInput) == "/quit" {
			AgentMemory.AddMessages(messages)
			AgentMemory.Save("./memory/agent-memory-saved-on-exit.json")
		...
		if strings.ToLower(userInput) == "/save" {
			AgentMemory.AddMessages(messages)
			AgentMemory.Save("./memory/agent-memory.json")
			continue
		}

		if strings.ToLower(userInput) == "/load" {
			AgentMemory.Load("./memory/agent-memory.json")
			messages = AgentMemory.Messages
			continue
		}

		if strings.ToLower(userInput) == "/resume" {
			AgentMemory.Load("./memory/agent-memory-saved-on-exit.json")
			messages = AgentMemory.Messages
			continue
		}
